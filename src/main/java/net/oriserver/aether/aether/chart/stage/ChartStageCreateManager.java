...
public class ChartStageCreateManager{
    ...
    public ChartStageCreateManager(JavaPlugin plugin, SQLiteManager sqLiteManager, ChartStageInfo chartStageInfo, ChartHologram chartHologram) {
        this.plugin = plugin;
    ...
    public void create(Player p, String stage_id) {
        if(createMap.containsKey(String.valueOf(p.getUniqueId())))p.sendMessage("作成中のステージがあります");
        else{
    ...
    public void quit(Player p) {
        if(!createMap.containsKey(String.valueOf(p.getUniqueId())))p.sendMessage("作成中のステージがありません");
        else{
    ...
    public ArrayList<Object> getStageData(String stage_id) {
        List<Object> stageData = chartStageDB.getDB("SELECT * FROM ChartStage WHERE stage_id = ?", Arrays.asList(stage_id), rs -> {
            ...
            while (rs.next()) {
                cd.add(rs.getString("stage_color"));
                cd.add(rs.getString("stage_name"));
                cd.add(rs.getDouble("stage_x"));
                cd.add(rs.getDouble("stage_y"));
                cd.add(rs.getDouble("stage_z"));
                ...
                cd.add(rs.getDouble("back_stage_x"));
                cd.add(rs.getDouble("back_stage_y"));
                cd.add(rs.getDouble("back_stage_z"));
                cd.add(rs.getDouble("back_stage_tp_x"));
                cd.add(rs.getDouble("back_stage_tp_y"));
                cd.add(rs.getDouble("back_stage_tp_z"));
                cd.add(rs.getFloat("back_stage_tp_yaw"));
                cd.add(rs.getFloat("back_stage_tp_pitch"));
                ...
                cd.add(rs.getDouble("hologram_stageName_x"));
                cd.add(rs.getDouble("hologram_stageName_y"));
                cd.add(rs.getDouble("hologram_stageName_z"));
                cd.add(rs.getDouble("hologram_time_x"));
                cd.add(rs.getDouble("hologram_time_y"));
                cd.add(rs.getDouble("hologram_time_z"));
    ...
    public void setData(ChartStageCreate createChartStage, ArrayList<Object> list, ArrayList<Double[]> checkPoint_List) {
        ...
        chartStageDB.setDB("INSERT OR IGNORE INTO ChartStage (stage_id,stage_color,stage_name,stage_x,stage_y,stage_z,stage_tp_x,stage_tp_y,stage_tp_z,stage_tp_yaw,stage_tp_pitch,"+
                "back_stage_x,back_stage_y,back_stage_z,back_stage_tp_x,back_stage_tp_y,back_stage_tp_z,back_stage_tp_yaw,back_stage_tp_pitch,"+
                "start_x,start_y,start_z,start_tp_x,start_tp_y,start_tp_z,start_tp_yaw,start_tp_pitch,goal_x,goal_y,goal_z,goal_tp_x,goal_tp_y,goal_tp_z,goal_tp_yaw,goal_tp_pitch,"+
                "hologram_stageName_x,hologram_stageName_y,hologram_stageName_z,hologram_time_x,hologram_time_y,hologram_time_z,star_time_3,star_time_2,star_time_1) "+
                "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);", list);
